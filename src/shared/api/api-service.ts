...
export const ApiService = {
    ...
    Users: {
        ...
        register: (login: string, email: string, password: string, bio: string) => 
            ApiClient.request<any>('/users', { method: 'PUT', body: JSON.stringify({ login, email, password, bio }) }),
        getById: (id: number) => 
            ApiClient.request<any>(`/users/${id}`),
        update: (login: string, email: string, bio: string, password?: string) => 
            ApiClient.request<any>('/users/update', { method: 'POST', body: JSON.stringify({ login, email, bio, password }) }),
        delete: () => 
            ApiClient.request<any>('/users', { method: 'DELETE' })
    },
